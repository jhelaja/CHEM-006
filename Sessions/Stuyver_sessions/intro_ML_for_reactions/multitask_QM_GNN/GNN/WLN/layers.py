...
class WLN_Layer(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        self.atom_features = layers.Dense(
            self.hidden_size,
            kernel_initializer=tf.random_normal_initializer(stddev=0.1),
            use_bias=False,
            input_shape=(50,),
        )
        self.nei_atom = layers.Dense(
            self.hidden_size,
            kernel_initializer=tf.random_normal_initializer(stddev=0.1),
            use_bias=False,
            input_shape=(
        ...
        self.nei_bond = layers.Dense(
            self.hidden_size,
            kernel_initializer=tf.random_normal_initializer(stddev=0.1),
            use_bias=False,
        )
        self.self_atom = layers.Dense(
            self.hidden_size,
            kernel_initializer=tf.random_normal_initializer(stddev=0.1),
            use_bias=False,
        )
        self.label_U2 = layers.Dense(
            self.hidden_size,
            activation=K.relu,
            kernel_initializer=tf.random_normal_initializer(stddev=0.1),
        )
        self.label_U1 = layers.Dense(
            self.hidden_size,
            activation=K.relu,
            kernel_initializer=tf.random_normal_initializer(stddev=0.1),
        )
    ...
    def call(self, graph_inputs):
        ...
        for i in range(self.depth):
            ...
            )  # (batch, #atoms, max_nb, hidden)
            fbond_nei = tf.gather_nd(
                input_bond, tf.dtypes.cast(bond_graph, tf.int64)
            )  # (batch, #atoms, max_nb, #bond features)
            ...
            mask_nei = K.reshape(
                tf.sequence_mask(
                    K.reshape(num_nbs, [-1]), self.max_nb, dtype=tf.float32
                ),
                [K.shape(input_atom)[0], -1, self.max_nb, 1],
            )
            f_nei = K.sum(
                h_nei * mask_nei, axis=-2, keepdims=False
            )  # (batch, #atoms, hidden) sum across atoms
            f_self = self.self_atom(atom_features)  # (batch, #atoms, hidden)

            layers.append(f_nei * f_self * self.node_reshape(node_mask))  # , -1))
            l_nei = K.concatenate(
                [fatom_nei, fbond_nei], axis=3
            )  # (batch, #atoms, max_nb, )
...
class Global_Attention(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        self.att_atom_feature = layers.Dense(
            self.hidden_size,
            kernel_initializer=tf.random_normal_initializer(stddev=0.1),
            use_bias=False,
            input_shape=(self.hidden_size,),
        )
        self.att_bin_feature = layers.Dense(
            self.hidden_size,
            kernel_initializer=tf.random_normal_initializer(stddev=0.1),
        )
        self.att_score = layers.Dense(
            1,
    ...
    def call(self, inputs, bin_features):
        ...
        att_hidden = K.relu(
            self.att_atom_feature(atom_pair) + self.att_bin_feature(bin_features)
        )
